...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			return array(
				...
				'rating'           => ($query->row['rating'] != null ? round($query->row['rating']) : 0),
	...
	public function getFastProducts($data = array()) {
		if (!empty($data['filter_category_id'])) {
			...
			if (!empty($data['filter_manufacturer_id'])) {
				$filter_manufacturer_id = $data['filter_manufacturer_id'];
			}else{
			...
			if (!empty($data['filter_name']) || !empty($data['filter_tag'])) {
				...
				if (!empty($data['filter_name'])) {
					...
					foreach ($words as $word) {
						$escape_word = $this->db->escape($word);
					...
					if (count($words) > 0) {
						if (count($words) == 1) {
							...
							$temp_table = "filter_name_".$this->session->getId()."_".time();
							$this->db->query("CREATE TEMPORARY TABLE ".$temp_table."(name varchar(255) NOT NULL) ENGINE=MyISAM  DEFAULT CHARSET=utf8");
							foreach($implode as $imp){
								$this->db->query("INSERT INTO ".$temp_table." SET name ='".$this->db->escape($imp)."'");
							}
						}
					...
					if (!empty($data['filter_description'])) {
						$filter_name .= " OR pd.description LIKE '%" . $escape_filter_name . "%'";
					}
				}
				...
				if (!empty($data['filter_tag'])) {
					...
					foreach ($words as $word) {
						$escape_word = $this->db->escape($word);
						$implode[] = "pd.tag LIKE N'%" . $escape_word . "%'";
					}
			...
			if ($temp_table != '') {
				$this->db->query("DROP TEMPORARY TABLE IF EXISTS ".$temp_table." IF EXISTS");
			}
